...
export const TOOLS: ToolDef[] = [
  ...
  // =========================================================================
  {
    ...
    description:
      "List Operations lost-item cases (items guests left at properties). Filter by status, property, assignee, or 'overdue' (open cases past their follow-up date). Defaults to open cases only.",
    input_schema: {
      type: "object",
      properties: {
        status: {
    ...
    execute: async (input) => {
      ...
      return cases.slice(0, limit).map((c) => ({
        ...
        conversation_url: c.conversation_url,
        source: c.source,
  ...
  },
  {
    name: "get_lost_item",
    description:
      "Full detail for a single lost-item case by id or case_number (LI-001023). Includes guest info, property, logistics, links, source/external IDs, and the activity timeline.",
    input_schema: {
      type: "object",
  ...
  },
  {
    ...
    description:
      "Open a new lost-item case. Required: item_description. Set property_id (preferred) or property_name. Always include guest_name + a way to reach them (email/phone). Optional cross-links: slack_thread_url, conversation_url. Status defaults to 'pending_pickup'. Use this when a guest tells the team they left something behind.",
    input_schema: {
      type: "object",
      properties: {
        ...
        slack_thread_url: {
          type: "string",
          description: "Optional Slack thread URL discussing this case.",
        },
        conversation_url: {
          type: "string",
          description:
            "Optional URL to the guest conversation (Hostaway, Airbnb, email, etc.).",
        },
        status: {
          type: "string",
          enum: [
    ...
    execute: async (input) => {
      const result = await lostItems.createCase({
        ...
        slack_thread_url: sOrNull(input.slack_thread_url) ?? null,
        conversation_url: sOrNull(input.conversation_url) ?? null,
  ...
  },
  {
    ...
    description:
      "Update fields on an existing lost-item case. Pass only the fields to change. Status transitions auto-stamp the matching milestone timestamp (picked_up → pickup_completed_at, delivered → delivered_at, completed → completed_at). Use this to record cleaning-vendor pickup, shipping carrier/tracking, return method, follow-up date, Slack/conversation links, etc.",
    input_schema: {
      type: "object",
      properties: {
        case_id: { type: "string" },
        fields: {
          ...
          additionalProperties: true,
          description:
            "Partial. Common keys: status, assigned_to, item_description, found_location, property_id, guest_name, guest_email, guest_phone, slack_thread_url, conversation_url, cleaning_vendor, return_method (shipped|guest_pickup|in_person|other), shipping_carrier, shipping_tracking, follow_up_date (YYYY-MM-DD), notes, external_url.",
        },
      },
  ...
  },
  {
    ...
    description:
      "Move a lost-item case to a new pipeline status. Pipeline: pending_pickup → picked_up → delivered → completed. Use 'failed' for cases that can't be recovered (guest never claimed, item lost in transit, duplicate, etc.).",
    input_schema: {
      type: "object",
      properties: {
        case_id: { type: "string" },
  ...
  {
    name: "add_lost_item_comment",
    description:
      "Add a comment to a lost-item case. Comments are first-class on the case detail page — use this to record what the guest said, vendor updates, decisions, or any context the team should see.",
    input_schema: {
      type: "object",
      properties: {
        case_id: { type: "string" },
